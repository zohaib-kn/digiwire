...
// Caps device pixel ratio so very high-DPI phones don't pay an
// unnecessary fill-rate cost for resolution beyond what's visible.
const MAX_DPR = 2;
...
export default function LineWaves({
  ...
  useEffect(() => {
    ...
    // frame and skip the continuous animation loop entirely.
    ...
    const resizeObserver = new ResizeObserver((entries) => {
      for (const entry of entries) {
        const w = entry.contentRect.width || container.offsetWidth || window.innerWidth;
        const h = entry.contentRect.height || container.offsetHeight || window.innerHeight;
        if (w === 0 || h === 0) continue;
    ...
    if (prefersReducedMotion) {
      // Render a single still frame instead of looping requestAnimationFrame.
      renderFrame(0);
    } else {
      animationFrameId = requestAnimationFrame(update);
    }
